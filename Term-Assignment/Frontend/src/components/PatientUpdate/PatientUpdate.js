...
const PatientUpdate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.patientId) validationErrors.patientId = 'Patient ID is required';
    ...
    try {
      const updatePayload = formData.fieldsToUpdate.reduce((acc, field) => {
      ...
      const response = await axios.put(`https://iz72w6q8ph.execute-api.us-east-1.amazonaws.com/dev/patients/${formData.patientId}`, payload, {
        headers: {
        ...
        <Col xs={12} md={8}>
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3" controlId="formPatientId">
              ...
              />
              {errors.patientId && <Form.Text className="text-danger">{errors.patientId}</Form.Text>}
            </Form.Group>
            
            <Form.Group className="mb-3" controlId="formFieldsToUpdate">
              <Form.Label className='label-update'>Select Fields to Update</Form.Label>
              {[
              'name', 
              'email', 
              'phoneNumber', 
            ...
            {formData.fieldsToUpdate.map((field) => (
              <Form.Group key={field} className="mb-3" controlId={`form${field}`}>
                <Form.Label className='label-update'>{field.charAt(0).toUpperCase() + field.slice(1)}</Form.Label>
                <Form.Control 
                  type="text" 
